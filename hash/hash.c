...
static const uint16_t crc16_ansi_tab[256] =
{        
...
static uint16_t crc16_ansi(const uint8_t *ptr, int length) {
...
static uint32_t crc_ObtenValorDeTabla(uint8_t VP_Pos_Tabla, uint32_t polynomial) {
...
uint32_t crc_body_reversed_false(uint8_t* data, uint16_t len, uint32_t code, uint32_t init, uint32_t final_xor) {
    for (int16_t VP_bytes = 0; VP_bytes < len; VP_bytes++) {
...
uint32_t select_crc(int hashid, uint8_t* data, uint16_t len) {
    switch (hashid) {
        ...
        default: LOG_ERR("Unsupport hash id: %d.\n", hashid);
    }
}